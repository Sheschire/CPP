...
//	delete(meta);
	...
	Brain	*brain;
	int	iter2;
	color("green", "Dogs are thinking :\n");
	for (iter2 = 0; iter2 < size / 2; iter2++)
	{
		brain = animalTab[iter2]->getBrain();
		brain->setIdeas("I'd like a bone...", 0);
		brain->setIdeas("My master is a nice guy !", 1);
		std::cout << animalTab[iter2]->getBrain()->getIdeas(0) << std::endl;
		std::cout << animalTab[iter2]->getBrain()->getIdeas(1) << std::endl;
	}
	color("green", "Cats are thinking :\n");
	while (iter2 < size)
	{
		brain = animalTab[iter2]->getBrain();
		brain->setIdeas("I'd like a fresh salmon...", 0);
		brain->setIdeas("My servant is a nice guy !", 1);
		std::cout << animalTab[iter2]->getBrain()->getIdeas(0) << std::endl;
		std::cout << animalTab[iter2]->getBrain()->getIdeas(1) << std::endl;
		iter2++;
	}
